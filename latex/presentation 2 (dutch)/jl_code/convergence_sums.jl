...
for n in nn
    for _ in 1:nsim
        error = abs(approx_rand(Integer(n)) - sol) + eps()
...
for n in nn
    for _ in 1:nsim
        error = abs(approx_rand2(Integer(n)) - sol) + eps()
        scatter!(p, (n, error), label="", color=:green, alpha=0.5, markersize=5)
    end
end

for n in nn
    error = abs(approx_det(Integer(n)) - sol) + eps()
    scatter!(p, (n, error), label="", color=:black, markersize=6)
end

for n in nn
    for _ in 1:nsim
        app, evals = approx_rand3(n / k, 1 / 2)
        error = abs(app - sol) + eps()
