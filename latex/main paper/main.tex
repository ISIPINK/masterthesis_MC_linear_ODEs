...
\begin{julia}[implementation of estimator of Example \ref{ex:semi heat source}]\label{jl:point estimator heat source}
  ...
  to sample until we do not sample it. This is geometrically distributed.
...
\begin{technique}[presampling the proces of \ref{jl:point estimator heat source}] \label{tech:presampling heat}
  ...
  As $\Delta \rightarrow 0$ $genPath$ converges, the dependence on $a_{0}$ dissapears and $a_{m}$ becomes the
  ...
  \begin{figure}[h!]
    ...
    \caption{$20$ samples of genPath$(x=0.5,t=0.15,dx=\{0.1,0.001\},a_{0}=0,a_{m} = 30)$. The source term needs to be sampled
