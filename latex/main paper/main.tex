...
\begin{definition}[?]
    \begin{align}
        y'                              & = A y  \Leftrightarrow                                                      \\
        y'+\sigma y                     & = (A + \sigma I) y \Leftrightarrow                                          \\
        e^{-\sigma t} ( e^{\sigma t}y)' & = (A + \sigma I) y    \Leftrightarrow                                       \\
        y(t)                            & = e^{-\sigma t} y(0) + \int_{0}^{t} e^{(s-t) \sigma} (A + \sigma I ) y(s)ds \\
    \end{align}
    Do following substitution: $e^{(s-t)\sigma} = \tau$:
    \begin{equation} \label{eq:poisson main}
        y(t) = \int_{0}^{e^{-\sigma t}}  y(0) d\tau + \int_{e^{-\sigma t}}^{1} \left(  \frac{A}{\sigma} + I\right)  y(s)d\tau
        .
    \end{equation}
    (\ref{eq:poisson main}) can be turned into a RRVE by sampling $\tau \sim U$, no Russian roulette is needed for
    termination because in the recursion the first integral get sampled with probability $1$.
