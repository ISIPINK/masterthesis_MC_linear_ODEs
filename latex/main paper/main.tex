...
be $O\left(h^{1.5} \right)$ in RMSE with $h$ the step size. Our
intuition for this behavior is based on how classical solvers
...
\begin{conjecture}[local RMSE RMC]
    ...
    \begin{equation}
        ...
        A(S) Y(S) + g(S),
    \end{equation}
    with $S \sim \text{Uniform}$ there holds
    \begin{equation}
...
One of the main things that could be done is rigor. We think
that RMC for IVPs is just an informal but easier way to think of an unbiased estimate of
the Von Neumann series approach.
...
Before reading \cite{ermakov_monte_2019} we
also derived a similar expression (probably with mistakes)
by using the law of total variance. This line
of reasoning wasn't something new see (16) in \cite{rath_ears_2022}.
...
the paper from NVIDIA \cite{kettunen_unbiased_2021} that
