...
\begin{technique}[SALT]
    ...
    an (bi-) orthonormal basis can be found by MC integration and MC integration
    can be accelerated by the current estimates of the coefficients.
    It can be seen as intrestring case of stochastic gradient descent.
\end{technique}
...
The current approach to non-linearity is inefficient (\ref{ex:exp int})
and is limited to small forms of non-linearity. Currently we haven't
tested any variance reduction techniques in that context.\\
% \begin{example}[fixpoint]
%     check period 1
% \end{example}


Branching tail recursion is little studied and hard. There  are multiple ways to
do branching tail recursion with each their advantages and disadvantages. In
the context of recursive MC there are $2$ techniques that
stand out.
...
\begin{example}[random ODEs] \label{ex:random ode}
    ...
    to the problem:
    \begin{align}
        E[f(Y(t))] & = E[E_A[f(Y(t)) \mid  A]] \\
                   & = E[E_A[f(Y(t))]]         \\
                   & = E[E_A[f(E_x[X(t,A)])]]  \\
    \end{align}
    We can estimate $f(E_x[X(t,A)])$ like (\ref{ex:exp int}).

    In our example we estimate the expectance and variance which can
