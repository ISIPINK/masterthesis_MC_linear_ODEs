...
\begin{example}[Dirichlet $y_{tt}=y$] \label{main dirichlet}
  ...
  $S \sim \text{Uniform}(b_{1},b_{0})$. In Figure \ref{fig:plots/main_dirichlet_viz.png} and \ref{fig:mainD explosion}
...
\begin{example}[\ref{def:discrete heat equation square} + source + coefficients] \label{ex:semi heat source}
  ...
  A similar derivation can be found for Example \ref{def:discrete heat equation square}
