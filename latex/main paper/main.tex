...
\begin{related}[main Poisson]
  ...
  Unlike \ref{jl:point estimator heat} the recursion proces can be reversed
...
\begin{technique}[presampling the proces of \ref{jl:point estimator heat source}]
  ...
  \begin{figure}[h!]
    ...
    \caption{$20$ samples of YvarPath$(x=0.5,t=0.15,dx=\{0.1,0.001\},a_{0}=30)$. The intermediate
    points in the path are where the source term need to be sampled. Note that the amount of
...
Similar to RMC for boundary Green's function we recursively sample first passages.
