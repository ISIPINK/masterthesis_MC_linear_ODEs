...
which can be done very smoothly,
creating resembling a multi-grid method. Alternately when reaching
...
\begin{pythonn}[implementation of (\ref{ex:RRMC IVP})] \label{py:RRMC IVP}
    ...
    We measured the convergence speed to be $O\left(\frac{D^{1.5}}{\sqrt{\text{nsim}}}
        \right)$.
    ...
    \begin{figure}[h!]
        ...
        \caption{Recursive calls of equation (\ref{eq:RRMC IVP outer})
            when calling $Y_{\text{out}}(3,D)$ $30$ times for different $D$.  }
        \label{fig:RRMC IVP}
    \end{figure}
\end{pythonn}

$1.5$ order of convergence is cool but this begs the question on how to achieve
higher order of convergence in $D$. Again it is easy to imitate classical methods
...
\begin{related}[CV RRMC]
    A very similar use for control variates for IVPs is \cite{daun_randomized_2011}.
\end{related}
