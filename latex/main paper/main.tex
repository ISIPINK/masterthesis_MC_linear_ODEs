...
mechanics of these Monte Carlo techniques to ODEs to explore
...
a Cauchy problem for large systems of linear differential
equations for describing queuing systems. Similarly to us
...
\begin{itemize}
    \item rendering and walk-on-sphere/first passage literature
          which contain many practical recursive
          Monte Carlo techniques,

    \item  Stochastic Gradient Descent literature
          that is connected through continuous gradient descent/Newton
          which we first encountered in \cite{huang_hybrid_2017},

    \item  information complexity literature which was
          unexpected to us, there are some intrestring
          biased algorithms applied on ODEs that achieve optimal
          information complexity rates in the randomized case,
          similar to us
          \citeauthor{daun_randomized_2011}'s \citeyear{daun_randomized_2011}
          \cite{daun_randomized_2011} uses control variates
          to achieve optimal order of convergence.
\end{itemize}

A recurrent theme in these fields is that informational
optimal algorithms and unbiased algorithms
are of theoretical importance and lead to more
efficient algorithms in the long run.
...
informally introducing recursive Monte Carlo
...
We conjecture that this method achieves optimal information
complexity.
...
Monte Carlo integration is an algorithm that
uses random sampling to estimate an integral.
...
\begin{definition}[Uniform Monte Carlo Integration]
    ...
    with the Best Linear Unbiased Estimators (BLUEs) for the average, Monte Carlo Integration
...
\begin{definition}[Root Mean Square Error (RMSE)]
    We define the RMSE of an estimator $\tilde{\theta}$ for $\theta$  as follows:
    \begin{equation}
...
limit us to $1$ dimensional unbiased estimators, so that RMSE becomes the variance.
It is easy to estimate the variance wich we use to
get confidence intervals via Chebyshev's inequality or
in the case of the BLUE via approximate normal distribution.  \\
...
If we limit ourselves to BLUEs with big sample size and
...
or complex domains. There is a linear trade off between average simulation
time and variance which allows us to define a relative efficiency statistic.
...
\begin{related}[Relative Monte Carlo Efficiency]
    For a reference see \cite{veach_robust_1997}.
\end{related}

For smooth $1$ dimensional integration is the linear trade off between
variance and average simulation time is not even close to optimal.
...
\begin{related}[Curse of Dimensionality]
...
\begin{related}[Monte Carlo Trapezoidal Rule]
    The optimal information complexity for integration in the
    randomized setting is known see \cite{heinrich_optimal_2001}.
\end{related}
