...
\begin{definition}[composite MC trapezoidal rule]
...
In the context of recursive Monte Carlo there are $2$ main ideas that
stand out:

\begin{technique}[tree regenerating]
    The structure of branching recursion can be captured by a tree. Generating
    the tree and storing it in memory can be expensive. Because in recursion
    you only need to retrace steps $1$ by $1$ it
\end{technique}

\begin{technique}[branching tail recursion]
...
\begin{related}
    ...
    We found our main ideas for branching tail recursion back in \cite{vicini_path_2021}.
    They applied on a non-branching estimator, which we think was overkill.
