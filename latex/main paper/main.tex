...
\begin{theorem}[Order Trapezoidal MC Rule] \label{thrm:order trap}
    The composite trapezoidal MC rule that uses on
    average $n$ function calls has
    \begin{equation}
...
\begin{proof}
    Start from equation (\ref{eq:MCtrap}). The RMSE is the variance
    so we can ignore addition by non random terms.
    \begin{align}
        \text{RMSE} & =\text{Var}\left( \sum_{x}  l B\left(\frac{1}{l}\right)
        \left(f(S_x)  - \frac{S_x}{\Delta x}(f(x+\Delta x) - f(x))\right)\right)
    \end{align}
\end{proof}

For the sake of simplicity, we only study polynomial integration:
