...
In this subsection, we introduce the recursive Monte Carlo
with following  initial value problem:
...
equation. If one were to implement equation (\ref{recursive RV}),
...
One issue with equation (\ref{python eps ydy}) is that the variance
increases rapidly as $t$ increases. However, this issue is resolved
in equation (\ref{ex:RRMC IVP}). It is important to note that equation
...
The primary motivating paper for this thesis is the work
by Sawhney et al. \citeauthor{sawhney_grid-free_2022}
...
complex boundary conditions. The focus of this thesis is to study the underlying
mechanics of these Monte Carlo techniques and their applications.
...
to Monte Carlo techniques and a short disccusion about a Monte Carlo trapezoidal
...
\begin{example}[Russian roulette] \label{ex:simple russian roulette}
    ...
    computational burden compared to $Z$. Although the variance of $\tilde{Z}$
    increases slightly compared to $Z$, it remains manageable.
...
\begin{definition}[MC trapezoidal rule]
    ...
    of function $f$ over the interval $[x, x+dx]$ as follows:

    \begin{equation}
        \int_{x}^{x+dx} f(s) , ds \approx
        \frac{f(x) + f(x+dx)}{2} + f(S_x) - f(x) - \frac{S_x - x}{dx}
        \left(f(x+dx) - f(x)\right),
    \end{equation}

    where $S_x$ is a random variable uniformly distributed between $x$ and $x+dx$.
\end{definition}
...
\begin{definition}[composite MC trapezoidal rule] \label{MCtrap}
    Definge the composite MC trapezoidal rule for approximating the integral
    of function $f$ over the interval $[a, b]$ with $n$ intervals
    and a Russian roulette rate $l$ as follows:

    \begin{align}
        \int_{a}^{b} f(s) , ds \approx & \sum_{x} \frac{f(x) + f(x+dx)}{2} \\
...
\begin{related}[MC trapezoidal rule]
    ...
    order for a MC trapezoidal rule is discussed. However,
    it is uncertain whether this improvement is similar to the one
    we have previously mentioned. Further examination and comparison
    would be needed to determine the similarities or differences between
    the two approaches. We expect that our approach requires more
    smoothness.
\end{related}
...
\begin{example}[$y'=y$ average condition]
    ...
    \begin{equation} \label{ydy_int}
        y' = y,
    \end{equation}

    but this time with different condition:
    ...
    we can form the following integral equation for (\ref{ydy_int}):

    \begin{equation} \label{int_ydy_int}
        y(t) = e - 1 + \int_{0}^{1} G(t,s) y(s) ds.
    \end{equation}

    Converting equation (\ref{int_ydy_int}) into a RRVE
    using RMC, we obtain:

    \begin{equation}\label{RRVE_ydy_int}
        Y(t) = e - 1 + 2B\left(\frac{1}{2}\right)Y(S)(H(t-S)+S-1),
    \end{equation}

    where $S \sim U$. We will skip the Python implementation
    of equation (\ref{RRVE_ydy_int}) as it does not provide any
    new information. Instead, we will plot realizations of
    equation (\ref{RRVE_ydy_int}) in Figure \ref{fig:ydy_int}.
