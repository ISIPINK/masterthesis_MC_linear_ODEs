...
\begin{related}[Monte Carlo Trapezoidal Rule]
    ...
    The optimal IBC for the determenistic, random and quantum case
...
then solving them. Our main tool for this are Green's functions. \\
...
the source term). In this context the Green's function are
like homogenous and particular solutions of a specific problem
...
To clarify Green's functions let's look at following example.
...
\begin{example}[$y_t=y$ average condition]
    ...
    The Green's function are constructed such that
...
\begin{example}[Dirichlet $y_{tt}=y$] \label{main dirichlet}
    We turn following ODE into Fredholm integral equation of
...
\begin{example}[coupled splitting on example \ref{main dirichlet}] \label{ex:coupled splitting}
    ...
    in various ways. (\ref{RRVE:coupled splitting}) is unbiased in following way:
...
\begin{pythonn}[implementation of (\ref{RRVE:coupled splitting})] \label{py:coupled splitting}
    ...
    \begin{figure}[h!]
        ...
        calling $X(0)$ once. We choose $X$ to have $20$ points and
...
be $O\left(h^{1.5} \right)$ in RMSE with $h$ the time step. \\
Our intuition for this behavior is based on how classical solvers
work and the MC trapezoidal rule see definition \ref{MCtrap}.

\begin{conjecture}[local RMSE RMC]
    Consider a general linear IVP
    \begin{equation}
        y_{t}(t)= A(t)y(t), y_{t_{0}},
    \end{equation}
    with $A$ a matrix then for the corresponding RRVE
    \begin{equation}
        y(t) \cong Y(t) = y(t_{0}) + h B \left( \frac{t-t_{0}}{h}\right) A(U(t-t_{0})+t_{0})
        Y(U(t-t_{0})+t_{0}),
    \end{equation}
    there holds
    \begin{equation}
...
\begin{example}[CV RRMC $y_t=y$]\label{ex:CV RRMC IVP}
    ...
    The implementation is very similar to TODO and
...
\begin{example}[random ODEs] \label{ex:random ode}
    ...
    \begin{equation}\label{eq:random ode}
        Y_t = AY, \quad Y(0)=1, \quad A \sim \text{Uniform}(0,1).
    \end{equation}
