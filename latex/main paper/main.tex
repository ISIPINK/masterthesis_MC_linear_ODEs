...
Stein's paradox for example. We will
limit us to $1$ dimensional unbiased estimators, so that MSE becomes the variance.
...
\begin{theorem}[Order Trapezoidal MC Rule] \label{thrm:order trap}
...
\end{theorem}
\textcolor{green}{
    TODO: I feel like I'm making a mistake against the law of total variance.
} \\
...
case, there is a loss of one order but the MC trapezoidal loses only a half order.
...
\begin{proof}
    ...
    \begin{align}
        \text{MSE} & =\text{Var}\left( \Delta x \sum_{j=1}^{n}  l B\left(\frac{1}{l}\right)
        \left(f(S_j)  - \frac{S_j -x_{j}}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j}))\right)\right)  \\
                   & =\Delta x^{2} \sum_{j=1}^{n} \text{Var}\left(   l B\left(\frac{1}{l}\right)
        \left(f(S_j)  - \frac{S_j -x_{j}}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j}))\right)\right).
    \end{align}
    We deal with the Russian roulette part with the law of total variance:
    \begin{equation}
        \operatorname{Var}(Y)=\mathrm{E}[\operatorname{Var}(Y \mid X)]+\operatorname{Var}(\mathrm{E}[Y \mid X]).
    \end{equation}
    If we assume the outcome of the Russian roulette it naturally removes some terms:
    \begin{align}
        \text{MSE} & =\Delta x^{2}\sum_{j=1}^{n}
        \frac{1}{l}\text{Var}
        \left(
        l\left(f(S_j)  - \frac{S_j -x_{j}}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j}))\right)
        \right)                   \nonumber                                                                                                          \\
                   & + \text{Var} \left(f(S_j)  - \frac{S_j -x_{j}}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j})) \right)                                  \\
                   & = (l+1)\Delta x^{2} \sum_{j=1}^{n} \text{Var} \left(f(S_j)  - \frac{S_j -x_{j}}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j})) \right) \\
                   & = (l+1)\Delta x^{2} \sum_{j=1}^{n} \text{Var} \left(f(S_j)  - \frac{S_j}{\Delta x}(f(x_{j}+\Delta x) - f(x_{j})) \right).
    \end{align}
    We substitute $S_j = \Delta x_{j} U_j + x_{j}$ with
    $U_j$ i.i.d. $U$ and then apply Taylor's theorem (remember we can ignore
    constants):

    \begin{align}
         & \frac{\text{MSE}}{(l+1)\Delta x^{2}}   \nonumber                                                                                  \\
         & =  \sum_{j=1}^{n} \text{Var} \left(f(\Delta x U_j + x_j)  - \frac{\Delta x U_j + x_j}{\Delta x}(f(x_j+\Delta x) - f(x_j)) \right) \\
         & =  \sum_{j=1}^{n} \text{Var} \left(f(\Delta x U_j + x_j)  -  U_j{\Delta x}(f(x_j+\Delta x) - f(x_j)) \right)                      \\
         & =  \sum_{j=1}^{n}
        \text{Var} \left(f(x_j)+\Delta x U_j f'(x_j) +\frac{\Delta x^{2}U^{2}_{j}}{2} f''(z_{1j})
        -  U_j \left(\Delta x f'(x_j) +\frac{\Delta x^{2}}{2}f''(z_{2j})\right) \right)                                                      \\
         & =  \sum_{j=1}^{n}
        \text{Var} \left(\frac{\Delta x^{2}U_j}{2} \left(U_j f''(z_{1j}) - f''(z_{2j}) \right)\right)                                        \\
         & = \Delta x^{4} \sum_{j=1}^{n}
        \text{Var} \left(\frac{U_j}{2} \left(U_j f''(z_{1j}) - f''(z_{2j}) \right)\right),
    \end{align}
    where $z_{1j} \in [x_j,S_j], z_{2j} \in [x_j, \Delta x+x_j]$. Define the smoothness
    factor
    \begin{equation}
        M^{2} = \max_{j}\text{Var} \left(\frac{U_j}{2} \left(U_j f''(z_{1j}) - f''(z_{2j}) \right)\right)   .
    \end{equation}
    We use the smoothness factor to bound the sum and replace $\Delta x = \frac{b-a}{n}$:

    \begin{align}
        \text{MSE} & \leq (l+1)\Delta x^{6} \sum_{j=1}^{n} M^{2} \\
                   & =(l+1) \frac{(b-a)^{6}}{n^{6}} n M^{2}      \\
                   & =(l+1) \frac{(b-a)^{6}}{n^{5}} M^{2}.
    \end{align}

    Equivalently the RMSE can be bounded finishing the proof:
    \begin{equation}
        \text{RMSE} \leq \sqrt{l+1}\frac{(b-a)^{3}}{n^{2.5}} M.
    \end{equation}
...
% don't need proof concept because of NVIDIA
% \vspace{0.2cm}
...
% that works in a markdown file. Keep use different headers 
% in markdown. Briefly explain your answers. 
%
% # Content tests
