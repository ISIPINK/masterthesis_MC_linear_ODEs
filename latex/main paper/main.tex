...
\begin{example}[nonlinear RRMC IVP] \label{ex:nonlinear RRMC IVP}
    ...
    \begin{align}
        y^2(t) & \approx y^2(t_n) + 2(t-t_n)y(t_n)y_t(t_n) + ((t-t_n)y_t(t_n))^2 + O((t-t_n)^2) \\
               & \approx y^2(t_n) + 2(t-t_n)y(t_n)y_t(t_n) + O((t-t_n)^2)
    \end{align}

    Integrating the control variate yields:

    \begin{align}
        \int_{t_{n}}^{t} & y^{2}(t_{n}) + 2(s-t_{n})y(t_{n})y_t(t_{n}) ds \\
                         & = (t-t_{n})y^{2}(t_{n})+
        2\left(\frac{t^{2}-t_{n}^{2}}{2} -t_{n}(t-t_{n}) \right)y(t_{n})y_t(t_{n}).
    \end{align}
