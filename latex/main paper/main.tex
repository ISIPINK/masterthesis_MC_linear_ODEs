...
\begin{abstract}
    ...
    This thesis introduces recursive Monte Carlo, mainly with examples on
    ODEs. (TODO)
\end{abstract}
...
complex boundary conditions. We focus us on studying the underlying
...
\begin{related}[MC for IVPs ODEs]
    ...
    \begin{itemize}
        ...
        \item \citeauthor{daun_randomized_2011}'s \citeyear{daun_randomized_2011}
              ...
              achieves order optimal order of convergence, in the sense
...
rate by $0.5$ order compared to the standard composite rule for each dimension,
...
\begin{related}[SALT]
    SALT gets discussed in \cite{gobet_new_2014}. Other papers noteworthy papers on
...
\begin{related}[MC trapezoidal rule]
    ...
    Further examination would be needed. We expect that
    our approach requires more smoothness.
\end{related}
...
MC, tree regrowing is a particularly noteworthy.
...
\begin{example}[$y''=y$ mixed boundary conditions]
    ...
    At this point, it may not be clear, but with these set of green functions,
...
\begin{pythonn}[implementation of (\ref{RRVE:coupled splitting})] \label{py:coupled splitting}
    ...
    \begin{figure}[h!]
        ...
        they are equally spaced and coupling is colored.
...
exploit locality and smoothness of the problem.
...
achieve higher order of convergence. This can be accomplished by eliminating
