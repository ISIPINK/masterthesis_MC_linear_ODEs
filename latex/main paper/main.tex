...
RVs are hard, there is no Pareto front we can draw.
...
Stein's paradox for example. We will almost always
limit ourselves to $1$ dimensional unbiased estimators
...
If we limit ourselves to (\ref{eq:BLUE}) with big sample size and
...
makes them well suited to a GPU implementation. These assumptions get you
pretty close to optimal solutions and are very practical for high dimensions
or complex domains. These assumptions and a linear trade off between average simulation
...
\begin{definition}[Monte Carlo Efficiency]
    Define Monte Carlo efficiency of an
...
Comparing better trade-offs is usually done with Information-Based Complexity (IBC).
...
In this subsection, we discuss how to turn BVPs into integral equations and
...
Our goal for this thesis was to learn about and explore some initial ideas we had.
We ended up limiting the scope to unbiased linear ODE solvers. We think that
we covered some ground in this domain but there is still much
obvious work to be done. \\

Maybe before we talk about that we should talk about the why.
We think that understanding and optimizing unbiased and deterministic linear ODE solvers
is the key to developing better randomized ODE solvers.
Randomized ODE solvers are useful for cases with little structure
where the advantage of IBC is significant.\\
...
\begin{example} \label{ex:random ode}
    ...
    so we can't sample $y(t,U)$ instead, we use unbiased estimates $Y(t,u)$ of samples
...
uses $1000$ times more function calls has the same IBC.  \\

One minor optimization that we were working on was the time process.
\begin{figure}[h!]
    \centering
...
A thing you could do is start the time process already half way such that
the average recursion calls drops but increasing the recursion calls is
not possible. An other issue with is the lack of control of the stationary
distribution. We think it would be better to spread the function calls
uniformly. \\

What we have in mind to solve these issues is using a Poisson point process
...
of recursion calls are Poisson distributed.

To deal with stiff problems in an unbiased way we would bet on exponential integrator
methods. The biggest obstacle to implementing them like diagonal RRMC is getting
unbiased estimates of exponential matrix-vector multiplication. Initially, we came
up with example \ref{ex:exp int} but some time later we found
the paper from NVIDIA \cite{kettunen_unbiased_2021} that optimizes the exponential
scalar case. There $2$ cases we are interested in, constant matrix and
variable matrix. This may allow direct sampling from the Magnus expansion.

RRMS is biased for non-linear problems \\
